...
#ifdef BCM_HARDWARE
static bool charging_override = false;
#endif
...
#ifdef BCM_HARDWARE
    // BCM2153 booting mode is available only via /proc/cmdline, so
    // so we can't use BOARD_CHARGING_MODE_BOOTING_LPM. 
    // BOARD_CHARGING_CMDLINE_NAME is also problematic, as ot doesn't 
    // allow for a separate lpm.rc.
    } else if (!strcmp(name,"BOOT_MODE")) {
        if (strcmp(value, "1") == 0)
            charging_override = true;
#endif
...
static int charging_mode_booting(void)
{
    ...
    ERROR("charging_override is: %d", charging_override);
    if (charging_override) {
        return 1;
    }
#endif
    return 0;
