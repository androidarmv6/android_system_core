...
int main(int argc, char **argv)
{
    ...
    INFO("property init\n");
    if (!is_charger)
#ifdef BOARD_CHARGING_CMDLINE_RECOVERY_VALUE && if defined(NO_RECOVERY_PARTITION)
...
#ifdef BOARD_CHARGING_CMDLINE_RECOVERY_VALUE
   /* BCM21553 devices (and possibly others) use kernel NV to identify
    * recovery mode. We always require a unique config for recovery.
    */
    if (strcmp(battchg_pause, BOARD_CHARGING_CMDLINE_RECOVERY_VALUE) == 0)
